...
class Tracker(object):
    def __init__(self, min_box_area: int = 10, aspect_ratio_thresh: float = 3.0):
    ...
    def tracking(self, dets_xyxy: np.ndarray, image_info: dict) -> tuple:
        ...
        if isinstance(dets_xyxy, np.ndarray) and len(dets_xyxy) > 0:
            ...
            bboxes_xyxy, ids, scores = self._tracker_update(dets_xyxy, image_info)
    ...
    def _tracker_update(self, dets: np.ndarray, image_info: dict):
        ...
        class_id = 0
        if dets is not None:
            online_targets = self.tracker.update(dets[:, :-1],
